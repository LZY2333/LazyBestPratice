...
该弹窗封装方式，在未推广的情况下前端同学也纷纷来抄作业，在项目中得到了大家的自发应用
...
返回一个pending的Promise，外部调用者可借此挂起后续流程，等待弹窗的结果返回

该pending状态的Promise，将由用户点击按钮等操作触发状态改变，返回数据继续外部后续流程

借由Promise实现每一次弹窗调用的发布订阅。
...
再`return Promise.resolve(false)`以结束该次调用的生命周期，
...
该方式能极大提升流程逻辑的可复用性，可读性，

注意子逻辑尽量使用纯函数，避免相互调用，只由主函数调用。

Promise的加入使得该方式能更灵活的拆分及组合，

Promise的拆分使用，让我能将用户DOM操作也视为异步操作，纳入主函数的逻辑组织。
...
这套最佳实践的一大创新是，将用户的界面操作纳入Promise流，从而抽象成和Promise请求一样的异步操作。
...
开篇就提到了，最佳实践，重要的不是这篇demo，而是自己的思想，

在编码过程中不断地问自己，怎么做能让使用者感觉更方便。
